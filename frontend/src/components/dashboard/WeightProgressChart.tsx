...
const WeightProgressChart: React.FC = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            if (user) {
                try {
                    const history = await health.getWeightHistory(user.id, 30);
                    // Process history for chart
                    // history is [{date: 'YYYY-MM-DD', weight: 80.5}, ...]

                    if (history.length > 0) {
                        const formattedData = history.map((item: any) => ({
                            name: new Date(item.date).toLocaleDateString('en-US', { month: 'short' }).toUpperCase(),
                            weight: item.weight,
                        ...
                        // Calculate change (vs first in period or previous week?)
                        // Simple logic: vs first in list
                ...
                <div className="flex justify-between items-start mb-6">
                    ...
                    <div className="text-right">
                        <p className="text-white text-2xl font-bold">
                            {currentWeight !== null ? currentWeight : '--'} <span className="text-sm font-normal text-white/50">kg</span>
                        </p>
                        <p className={`${weightChange <= 0 ? 'text-emerald-400' : 'text-red-400'} text-xs font-medium flex items-center justify-end gap-1`}>
