...
export default function AIChat() {
  ...
  const handleSend = async () => {
    ...
    if (!user?.id) {
      setMessages((prev) => [
        ...prev,
        { role: 'assistant', text: 'Користувач ще не синхронізований. Спробуй трохи пізніше.' },
      ]);
      return;
    }
    ...
    try {
      const data = await chatApi(user.id, userMsg);
      ...
      setMessages((prev) => [
        ...prev,
        { role: 'assistant', text: 'Вибач, щось пішло не так.' },
      ]);
    }
  };
